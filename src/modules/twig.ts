...
twigComposer.command(['settwig'], async (ctx) => {
  let now = await getPing(ctx);
  if (ctx.chat.private) {
...
twigComposer.command(['deltwig'], async (ctx) => {
  let now = await getPing(ctx);
  try {
    if (ctx.chat.private) {
      let text = `🌱 This command only work in **group/supergroup!**`;
